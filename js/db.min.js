    ...
    },
    {
      ...
      value: function _fetchGenres() {
        ...
        fetch(url)
          ...
          .then(function (data) {
            var t = wpd_base.token ? data.data : data
            if (t.length) {
              _this.genre_list = t
              _this._renderGenre()
            }
      ...
      value: function _handleGenreClick() {
        ...
        this.on('click', '.db--genreItem', function (t) {
          ...
          if (self.classList.contains('is-active')) {
            var index = _this2.genre.indexOf(self.innerText)
            self.classList.remove('is-active')
            _this2.genre.splice(index, 1)
            _this2.paged = 1
          ...
          _this2.genre.push(self.innerText)
          _this2.paged = 1
          _this2.finished = false
          _this2.subjects = []
          _this2._fetchData()
        ...
        document.querySelector('.db--genres').innerHTML = this.genre_list
          .map(function (item) {
            return '<span class="db--genreItem'
              .concat(
                _this3.genre_list.includes(item.name) ? ' is-active' : '',
                '">'
              )
    ...
    },
    {
      ...
      value: function _randerDateTemplate() {
        ...
        for (var key in result) {
          var date = key.split('-')
          html +=
            '<div class="db--listBydate"><div class="db--titleDate JiEun"><div class="db--titleDate__day">'
        ...
        document.querySelector('.db--list').innerHTML = this.subjects
          .map(function (item) {
            return '<div class="db--item">'
              ...
              .concat(item.create_time, '</div><div class="db--score JiEun">')
              .concat(
                item.douban_score > 0
                  ? '<svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" ><path d="M12 20.1l5.82 3.682c1.066.675 2.37-.322 2.09-1.584l-1.543-6.926 5.146-4.667c.94-.85.435-2.465-.799-2.567l-6.773-.602L13.29.89a1.38 1.38 0 0 0-2.581 0l-2.65 6.53-6.774.602C.052 8.126-.453 9.74.486 10.59l5.147 4.666-1.542 6.926c-.28 1.262 1.023 2.26 2.09 1.585L12 20.099z"></path></svg>' +
                      item.douban_score
    ...
    },
    {
      ...
      value: function _handleNavClick() {
        ...
        this.on('click', '.db--navItem', function (t) {
          if (t.target.classList.contains('current')) {
            return
          }
          _this6.genre = []
          _this6.type = t.target.dataset.type
          if (_this6.type != 'book') {
            _this6._fetchGenres()
